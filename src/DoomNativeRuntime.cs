...
internal sealed class DoomNativeRuntime : IDisposable
{
    ...
    internal bool Start()
    {
        ...
        try
        {
            ...
            int audioStatus = _audioStatus?.Invoke() ?? 0;
            MelonLogger.Msg($"Doom TV: native audio status: sound effects={((audioStatus & 1) != 0 ? "ready" : "unavailable")}, music backend={((audioStatus & 2) != 0 ? "ready" : "unavailable")}, music playback={((audioStatus & 4) != 0 ? "active" : "inactive")}.");
            return true;
        }
        catch (Exception ex)
        {
